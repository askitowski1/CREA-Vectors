...
#can't concat a list so asigning this as an object
inst_word = word_text
...
results_list = [{ 'word': inst_word, }]

#letting the user click the button
#pressing the keys works as intended
#clicking goes to a random category and doesn't save it properly
#I want it to be able to work with both but even if you do only clicking it doesn't work
while True:
    response = None
...
#                    'rating': response
#                })
                results_list[0][inst_cat] = response
                cat_index += 1
                inst_cat = category_list[cat_index]
